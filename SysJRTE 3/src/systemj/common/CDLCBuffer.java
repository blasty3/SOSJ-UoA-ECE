...
public class CDLCBuffer {
   ...
   // private static final Object MigrationDetailsLock = new Object();
    ...
    public static Hashtable GetCDLocTempBuffer(){
         synchronized(CDSSLocBufferLock){
    ...
    public static void AddChanReconfig(String chcdname, String cname, String cdirection, String partnercdname, String partnercname){
        synchronized(ChanReconfigureBufferLock){
            if(ChanReconfigureBuffer.containsKey("input")|| ChanReconfigureBuffer.containsKey("output")){
                
                if(ChanReconfigureBuffer.containsKey("input")){
                    Hashtable hash1 = (Hashtable)ChanReconfigureBuffer.get("input");
                    
                    Hashtable hash2 = new Hashtable();
                    
                    hash2.put("partnercdname", partnercdname);
                    hash2.put("partnercname",partnercname);
                    hash2.put("cdname",chcdname);
                    
                    hash1.put(cname, hash2);
                    
                    ChanReconfigureBuffer.put("input", hash1);
                    
                }
                
                if(ChanReconfigureBuffer.containsKey("output")){
                    Hashtable hash1 = (Hashtable)ChanReconfigureBuffer.get("output");
                    ...
                    ChanReconfigureBuffer.put("output", hash1);
                }
                ...
                hash2.put("cdname",chcdname);
                hash1.put(cname, hash2);
                ChanReconfigureBuffer.put(cdirection, hash1);
    ...
    think of remodifying the data structure of the ChanReconfigBuffer
    */
    public static void AddChanReconfig(String chcdname, String cname, String cdirection, String partnercdname, String partnercname){
        synchronized(ChanReconfigureBufferLock){
   ...
   public static void RemoveRequestUpdateCD(int i){
    ...
    public static void TransferRequestKillCDToBuffer(Vector vec){
        synchronized(RequestKillCDListLock){
            
            for (int i=0;i<vec.size();i++){
              RequestKillCDList.addElement(vec.get(i).toString());
           }
